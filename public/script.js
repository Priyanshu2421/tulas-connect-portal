...
document.addEventListener('DOMContentLoaded', () => {
                ...
                return `<div class="border p-4 rounded-md">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            ${r.status === 'Denied' ? `<p class="text-sm text-red-700 mt-1"><strong>Denial Reason:</strong> ${r.denialReason}</p>` : ''}
                        </div>
            ...
            document.getElementById('leave-request-form').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        showStudentLeave(); // Refresh the page
                    } else {
            ...
            document.querySelectorAll('.approve-leave-btn, .deny-leave-btn').forEach(btn => {
                btn.addEventListener('click', async e => {
                    ...
                    if (action === 'Denied') {
                        ...
                        if (denialReason === null) return; // User cancelled
                    }
    ...
    function renderEditableTimetable(type, id, data) {
        ...
        editContainer.innerHTML = `<h3 class="text-xl font-bold mb-4">Editing Timetable for ${type === 'student' ? `Batch ${id}` : `Faculty ${id}`}</h3><div class="overflow-x-auto"><table id="editable-timetable" class="min-w-full border-collapse"><thead><tr><th class="p-1 border">Day/Time</th>${headerRow}</tr></thead><tbody>${bodyRows}</tbody></table></div><div class="mt-4"><button id="save-timetable-btn" class="bg-green-600 text-white px-4 py-2 rounded-lg">Save Changes</button><button id="cancel-edit-timetable-btn" class="bg-gray-500 text-white px-4 py-2 rounded-lg">Back</button></div>`;
