...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function showStudentIdCard() {
        ...
        try {
            ...
            if (statusData.status === 'Not Applied' || statusData.status === 'Denied') {
                formHTML = `<form id="id-card-form"><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label class="block">Full Name</label><input type="text" name="name" value="${profile.name || ''}" class="w-full mt-1 p-2 border rounded bg-gray-100" readonly></div><div><label class="block">Phone Number</label><input type="tel" name="phone" value="${profile.phone || ''}" required class="w-full mt-1 p-2 border rounded"></div><div><label class="block">Program</label><input type="text" name="program" value="${profile.program || ''}" class="w-full mt-1 p-2 border rounded bg-gray-100" readonly></div><div><label class="block">Department</label><input type="text" name="department" value="${profile.department || ''}" class="w-full mt-1 p-2 border rounded bg-gray-100" readonly></div><div><label class="block">Date of Birth</label><input type="date" name="dob" value="${profile.dob || ''}" required class="w-full mt-1 p-2 border rounded"></div><div><label class="block">Blood Group</label><input type="text" name="bloodGroup" value="${profile.bloodGroup || ''}" required class="w-full mt-1 p-2 border rounded"></div><div class="md:col-span-2"><label class="block">Upload Your Photo (Max 12MB)</label><input type="file" name="idCardPhoto" id="idCardPhotoInput" accept="image/*" class="w-full mt-1 p-2 border rounded file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100" required><input type="hidden" name="userId" value="${profile.id || ''}"></div></div><div class="mt-6"><button type="submit" class="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700">Submit Application</button></div></form>`;
            } else {
            ...
            mainContent.innerHTML = `
                <div class="bg-white p-8 rounded-lg shadow-lg space-y-8">
                    <div>
                        ...
                        <form id="leave-request-form" class="space-y-4 p-4 border rounded-lg bg-gray-50">
                            <div>
                                <label for="leave-start-date" class="block text-sm font-medium text-gray-700">Start Date</label>
                                <input type="date" id="leave-start-date" name="startDate" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label for="leave-end-date" class="block text-sm font-medium text-gray-700">End Date</label>
                                <input type="date" id="leave-end-date" name="endDate" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label for="leave-reason" class="block text-sm font-medium text-gray-700">Reason for Leave</label>
                                <textarea id="leave-reason" name="reason" rows="3" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md"></textarea>
                            </div>
                            <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">Submit Request</button>
                        </form>
                    </div>
                    <div>
                        <h2 class="text-2xl font-bold mb-4">My Leave History</h2>
    ...
    async function showFacultyAssignments() {
        ...
        try {
            ...
            const assignmentsHtml = assignments.map(asg => {
                ...
                return `<div class="border p-4 rounded-lg"><div class="flex justify-between items-start"><div><h3 class="text-xl font-semibold">${asg.title}</h3><p class="text-gray-600 my-1">${asg.description}</p><p class="text-sm text-red-600">Due: ${new Date(asg.dueDate).toLocaleDateString()}</p>${asg.filePath ? `<a href="${API_BASE_URL}${asg.filePath}" target="_blank" class="text-green-600 hover:underline">View Assignment File</a>` : ''}</div><span class="text-sm font-bold bg-gray-200 px-3 py-1 rounded-full">${asgSubmissions.length} Submissions</span></div><hr class="my-3"><div class="space-y-2">${asgSubmissions.length > 0 ? asgSubmissions.map(sub => `<div class="flex justify-between items-center p-2 rounded-md ${sub.status === 'Approved' ? 'bg-green-50' : sub.status === 'Denied' ? 'bg-red-50' : 'bg-gray-50'}"><div><p class="font-medium">${sub.studentName} (${sub.studentId})</p><a href="${API_BASE_URL}${sub.filePath}" target="_blank" class="text-sm text-blue-600 hover:underline">View Submission</a></div><div>${sub.status === 'Pending' ? `<button class="approve-btn bg-green-500 text-white px-2 py-1 text-sm rounded" data-id="${sub.id}">Approve</button> <button class="deny-btn bg-red-500 text-white px-2 py-1 text-sm rounded" data-id="${sub.id}">Deny</button>` : `<span class="font-bold ${sub.status === 'Approved' ? 'text-green-700' : 'text-red-700'}">${sub.status}</span>`}</div></div>`).join('') : '<p class="text-gray-500 text-sm">No submissions yet.</p>'}</div></div>`;
            }).join('');

            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><div class="flex justify-between items-center mb-6"><h2 class="text-2xl font-bold">Manage Assignments</h2><button id="add-assignment-btn" class="bg-green-600 text-white px-4 py-2 rounded-lg">Add New Assignment</button></div><div id="add-assignment-form-container" class="hidden mb-6 p-4 border rounded-lg bg-gray-50"><form id="add-assignment-form"><input type="text" name="title" placeholder="Title" required class="w-full p-2 border rounded mb-2"><textarea name="description" placeholder="Description" required class="w-full p-2 border rounded mb-2"></textarea><label>Due Date: <input type="date" name="dueDate" required class="p-2 border rounded mb-2"></label><label>Assignment File (Optional): <input type="file" name="assignmentFile" class="w-full p-2 border rounded"></label><button type="submit" class="bg-blue-600 text-white px-4 py-2 mt-2 rounded-lg">Create</button></form></div><div class="space-y-6">${assignmentsHtml}</div></div>`;
    ...
    async function showFacultyLeave() {
        ...
        try {
            ...
            const requestsHtml = requests.length > 0 ? requests.map(r => {
                ...
                let actionButtons = '';
                if (r.status === 'Pending') {
                    actionButtons = `<button class="approve-leave-btn bg-green-500 text-white px-3 py-1 text-sm rounded" data-id="${r.id}">Approve</button>
                                     <button class="deny-leave-btn bg-red-500 text-white px-3 py-1 text-sm rounded" data-id="${r.id}">Deny</button>`;
                } else {
                    actionButtons = `<span class="text-sm font-bold">Resolved by ${r.resolvedBy || 'N/A'}</span>`;
                }
            ...
            document.querySelectorAll('.approve-leave-btn, .deny-leave-btn').forEach(btn => {
                btn.addEventListener('click', async e => {
                    ...
                    const action = e.target.classList.contains('approve-leave-btn') ? 'Approved' : 'Denied';
                    let denialReason = null;

                    if (action === 'Denied') {
                        denialReason = prompt('Please provide a reason for denial (optional):');
    ...
    async function showFacultyAttendance() {
        mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Mark Attendance</h2><div class="flex gap-4 mb-4"><input type="date" id="attendance-date" class="p-2 border rounded"><select id="attendance-class" class="p-2 border rounded"><option>CS-301</option><option>CS-501</option><option>CS-502</option></select><button id="load-students-btn" class="bg-blue-600 text-white px-4 py-2 rounded-lg">Load Students</button></div><div id="student-list-container"></div></div>`;
        ...
        document.getElementById('load-students-btn').addEventListener('click', async () => {
            ...
            try {
                ...
                const studentRows = users.map(s => `<tr class="student-row border-b" data-student-id="${s.id}"><td class="p-2">${s.name}</td><td class="p-2">${s.id}</td><td class="p-2"><label class="mr-4"><input type="radio" name="status-${s.id}" value="P" checked> Present</label><label><input type="radio" name="status-${s.id}" value="A"> Absent</label></td></tr>`).join('');

                container.innerHTML = `<table class="min-w-full bg-white"><thead><tr class="bg-gray-100"><th class="p-2 text-left">Name</th><th class="p-2 text-left">ID</th><th class="p-2 text-left">Status</th></tr></thead><tbody>${studentRows}</tbody></table><button id="submit-attendance-btn" class="mt-4 bg-green-600 text-white px-6 py-2 rounded-lg">Submit Attendance</button>`;
    ...
    async function showFacultyMarks() {
         mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Enter Marks</h2><div class="flex gap-4 mb-4"><select id="marks-subject" class="p-2 border rounded"><option value="CS-501 - Algorithms">CS-501 - Algorithms</option><option value="CS-502 - Database Systems">CS-502 - Database Systems</option><option value="CS-503 - Operating Systems">CS-503 - Operating Systems</option></select><button id="load-marks-students-btn" class="bg-blue-600 text-white px-4 py-2 rounded-lg">Load Students</button></div><div id="marks-student-list-container"></div></div>`;
        ...
        document.getElementById('load-marks-students-btn').addEventListener('click', async () => {
             ...
             try {
                ...
                const studentRows = users.map(s => `<tr class="marks-row border-b" data-student-id="${s.id}"><td class="p-2">${s.name} (${s.id})</td><td class="p-2"><input type="number" placeholder="Marks" class="marks-obtained w-24 p-1 border rounded"></td><td class="p-2"><input type="number" value="100" class="max-marks w-24 p-1 border rounded"></td><td class="p-2"><input type="number" placeholder="GP" class="grade-point w-24 p-1 border rounded"></td></tr>`).join('');

                 container.innerHTML = `<table class="min-w-full bg-white"><thead><tr class="bg-gray-100"><th class="p-2 text-left">Student</th><th class="p-2 text-left">Marks Obtained</th><th class="p-2 text-left">Max Marks</th><th class="p-2 text-left">Grade Point</th></tr></thead><tbody>${studentRows}</tbody></table><button id="submit-marks-btn" class="mt-4 bg-green-600 text-white px-6 py-2 rounded-lg">Submit Marks</button>`;
    ...
    async function showFacultySearch() {
        mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Search Student</h2><div class="flex gap-2"><input type="text" id="student-search-input" placeholder="Enter Student ID" class="flex-grow p-2 border rounded"><button id="student-search-btn" class="bg-blue-600 text-white px-6 py-2 rounded-lg">Search</button></div><div id="student-search-results" class="mt-6"></div></div>`;
        ...
        document.getElementById('student-search-btn').addEventListener('click', async () => {
            ...
            try {
                ...
                const { analytics } = await analyticsRes.json();
                 ...
                 resultsContainer.innerHTML = `<div class="border p-4 rounded-lg"><h3 class="text-xl font-bold">${profile.name} (${profile.id})</h3><p>${profile.department} - ${profile.course}</p><hr class="my-3"><div class="grid grid-cols-1 md:grid-cols-3 gap-4"><div class="text-center p-2 bg-gray-100 rounded"><p class="text-sm">CGPA</p><p class="font-bold text-2xl">${analytics.cgpa}</p></div><div class="text-center p-2 bg-gray-100 rounded"><p class="text-sm">Attendance</p><p class="font-bold text-2xl">${analytics.attendancePercentage}%</p></div><div class="text-center p-2 bg-gray-100 rounded"><p class="text-sm">Overall %</p><p class="font-bold text-2xl">${analytics.overallPercentage}%</p></div></div></div>`;
            } catch (error) {
    ...
    async function showFacultyMLInsights() {
        ...
        try {
            ...
            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Student Performance Prediction</h2><p class="text-sm text-gray-500 mb-4">This is a simplified prediction based on past CGPA trends to identify students at risk.</p><div class="overflow-x-auto"><table class="min-w-full"><thead class="bg-gray-100"><tr><th class="p-2 text-left">Student ID</th><th class="p-2 text-center">Last CGPA</th><th class="p-2 text-center">Predicted Next Sem CGPA</th><th class="p-2 text-center">Risk Level</th></tr></thead><tbody>${predictions.map(p => `<tr class="border-b"><td class="p-2">${p.studentId}</td><td class="p-2 text-center">${p.lastCgpa}</td><td class="p-2 text-center font-bold">${p.predictedCgpa}</td><td class="p-2 text-center"><span class="px-3 py-1 text-xs rounded-full ${p.risk === 'High' ? 'bg-red-200 text-red-800' : p.risk === 'Medium' ? 'bg-yellow-200 text-yellow-800' : 'bg-green-200 text-green-800'}">${p.risk}</span></td></tr>`).join('')}</tbody></table></div></div>`;
        } catch (error) {
    ...
    async function showHODDashboard() {
        ...
        const user = JSON.parse(sessionStorage.getItem('loggedInUser'));
        try {
            const res = await fetch(`${API_BASE_URL}/hod/dashboard/${encodeURIComponent(user.department)}`);
            const { data } = await res.json();

            mainContent.innerHTML = `<div class="space-y-8"><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"><div class="bg-white p-6 rounded-lg shadow-lg text-center"><h3 class="text-lg font-semibold text-gray-500">Total Students</h3><p class="text-4xl font-bold text-blue-600 mt-2">${data.totalStudents}</p></div><div class="bg-white p-6 rounded-lg shadow-lg text-center"><h3 class="text-lg font-semibold text-gray-500">Total Faculty</h3><p class="text-4xl font-bold text-purple-600 mt-2">${data.totalFaculty}</p></div><div class="bg-white p-6 rounded-lg shadow-lg text-center"><h3 class="text-lg font-semibold text-gray-500">Avg. Attendance</h3><p class="text-4xl font-bold text-orange-500 mt-2">${data.avgAttendance}%</p></div><div class="bg-white p-6 rounded-lg shadow-lg text-center"><h3 class="text-lg font-semibold text-gray-500">Avg. CGPA</h3><p class="text-4xl font-bold text-green-600 mt-2">${data.avgCgpa}</p></div></div><div class="bg-white p-6 rounded-lg shadow-lg"><h3 class="text-xl font-bold mb-4">Programs Offered</h3><div class="overflow-x-auto"><table class="min-w-full"><thead class="bg-gray-100"><tr><th class="p-2 text-left">Program</th><th class="p-2 text-left">Degree</th><th class="p-2 text-left">Specialization</th></tr></thead><tbody>${data.programs.map(p => `<tr class="border-b"><td class="p-2">${p.programName}</td><td class="p-2">${p.degree}</td><td class="p-2">${p.specialization}</td></tr>`).join('')}</tbody></table></div></div></div>`;
        } catch (error) {
    ...
    async function showHODFaculty() {
        ...
        const user = JSON.parse(sessionStorage.getItem('loggedInUser'));
        try {
            const res = await fetch(`${API_BASE_URL}/users?role=Faculty&department=${encodeURIComponent(user.department)}`);
            const { users } = await res.json();
             mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Faculty Members - ${user.department}</h2><div class="overflow-x-auto"><table class="min-w-full"><thead class="bg-gray-100"><tr><th class="p-2 text-left">Name</th><th class="p-2 text-left">Email</th><th class="p-2 text-left">Phone</th></tr></thead><tbody>${users.map(f => `<tr class="border-b"><td class="p-2">${f.name}</td><td class="p-2">${f.email}</td><td class="p-2">${f.phone}</td></tr>`).join('')}</tbody></table></div></div>`;
        } catch (error) {
    ...
    async function showAdminManageUsers() {
        ...
        try {
            ...
            const renderTable = (filteredUsers) => {
                document.getElementById('user-table-container').innerHTML = `<table class="min-w-full"><thead class="bg-gray-100"><tr><th class="p-2 text-left">Name</th><th class="p-2 text-left">User ID</th><th class="p-2 text-left">Role</th><th class="p-2 text-left">Department</th><th></th></tr></thead><tbody>${filteredUsers.map(u => `<tr class="border-b"><td class="p-2">${u.name}</td><td class="p-2">${u.id}</td><td class="p-2">${u.role}</td><td class="p-2">${u.department || 'N/A'}</td><td class="p-2"><button class="delete-user-btn text-red-500" data-id="${u.id}" data-name="${u.name}">Delete</button></td></tr>`).join('')}</tbody></table>`;
            ...
            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Manage Users</h2><div class="mb-4"><select id="role-filter" class="p-2 border rounded"><option value="">All Roles</option><option>Student</option><option>Faculty</option><option>Admin</option><option>HOD</option></select></div><div id="user-table-container" class="overflow-x-auto"></div></div>`;
    ...
    async function showAdminTimetables() {
        ...
        departmentSelect.addEventListener('change', async (e) => {
            const selectedDepartment = e.target.value;
            courseContainer.innerHTML = ''; courseContainer.classList.add('hidden');
            yearContainer.innerHTML = ''; yearContainer.classList.add('hidden');
            selectionContainer.innerHTML = '';

            if (!selectedDepartment) return;

            courseContainer.innerHTML = '<p>Loading courses...</p>'; courseContainer.classList.remove('hidden');
            ...
            try {
                ...
                if (data.programs && data.programs.length > 0) {
                    ...
                    courseContainer.innerHTML = `<label for="timetable-course-filter" class="block text-sm font-medium text-gray-700 mb-1">Step 2: Course</label><select id="timetable-course-filter" class="w-full p-2 border rounded-md shadow-sm"><option value="">-- Select a Course --</option>${courseOptions}</select>`;
        ...
        const handleCourseSelection = () => {
            ...
            selectionContainer.innerHTML = ''; yearContainer.classList.add('hidden');
            if (!selectedCourse) return;
            yearContainer.innerHTML = `<label for="timetable-year-filter" class="block text-sm font-medium text-gray-700 mb-1">Step 3: Year</label><select id="timetable-year-filter" class="w-full p-2 border rounded-md shadow-sm"><option value="">-- Select a Year --</option><option value="1">1st Year</option><option value="2">2nd Year</option><option value="3">3rd Year</option><option value="4">4th Year</option></select>`;
        ...
        const handleYearSelection = async () => {
            ...
            try {
                ...
                if (batches.length > 0) {
                    studentHtml += '<div class="space-y-2">' + batches.map(batch => `<div class="p-3 bg-gray-50 rounded-lg flex justify-between items-center"><span>Batch: ${batch}</span><button class="edit-timetable-btn bg-blue-500 text-white px-3 py-1 text-sm rounded-md" data-type="student" data-id="${batch}">Edit</button></div>`).join('') + '</div>';
                } else {
                ...
                if (faculty.length > 0) {
                    facultyHtml += '<div class="space-y-2">' + faculty.map(f => `<div class="p-3 bg-gray-50 rounded-lg flex justify-between items-center"><span>${f.name} (${f.id})</span><button class="edit-timetable-btn bg-blue-500 text-white px-3 py-1 text-sm rounded-md" data-type="faculty" data-id="${f.id}">Edit</button></div>`).join('') + '</div>';
                } else {
    ...
    function renderEditableTimetable(type, id, data) {
        ...
        const bodyRows = Object.keys(data.schedule).map(day => {
            const daySlots = data.schedule[day].map((slot, index) => {
                if (isStudent) {
                    ...
                    return `<td class="p-1 border"><input type="text" placeholder="Subject" class="w-full p-1 border rounded text-sm mb-1" data-day="${day}" data-index="${index}" data-field="subject" value="${subject}"><input type="text" placeholder="Faculty" class="w-full p-1 border rounded text-sm" data-day="${day}" data-index="${index}" data-field="faculty" value="${faculty}"></td>`;
                } else {
                    const value = slot || '';
        ...
        editContainer.innerHTML = `<h3 class="text-xl font-bold mb-4">Editing Timetable for ${type === 'student' ? `Batch ${id}` : `Faculty ${id}`}</h3><div class="overflow-x-auto"><table id="editable-timetable" class="min-w-full border-collapse"><thead><tr><th class="p-1 border">Day/Time</th>${headerRow}</tr></thead><tbody>${bodyRows}</tbody></table></div><div class="mt-4"><button id="save-timetable-btn" class="bg-green-600 text-white px-4 py-2 rounded-lg">Save Changes</button><button id="cancel-edit-timetable-btn" class="ml-2 bg-gray-500 text-white px-4 py-2 rounded-lg">Back</button></div>`;

        document.getElementById('cancel-edit-timetable-btn').addEventListener('click', () => {
            editContainer.classList.add('hidden');
            document.getElementById('timetable-main-view').classList.remove('hidden');
        });
    ...
    async function showAdminIdRequests() {
         ...
         try {
            ...
            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">ID Card Requests</h2><div class="space-y-4">${requests.length > 0 ? requests.map(r => `<div class="border rounded-lg p-4 flex flex-col md:flex-row items-start gap-4"><img src="${API_BASE_URL}${r.photoUrl}" class="w-24 h-24 object-cover rounded-md flex-shrink-0"><div class="flex-grow"><p class="font-bold text-lg">${r.userName} (${r.userId})</p><div class="grid grid-cols-2 gap-x-4 gap-y-1 text-sm mt-2"><p><strong>Phone:</strong> ${r.phone || 'N/A'}</p><p><strong>DOB:</strong> ${new Date(r.dob).toLocaleDateString() || 'N/A'}</p><p><strong>Program:</strong> ${r.program || 'N/A'}</p><p><strong>Blood Grp:</strong> ${r.bloodGroup || 'N/A'}</p><p class="col-span-2"><strong>Department:</strong> ${r.department || 'N/A'}</p></div></div><div class="flex flex-col gap-2 w-full md:w-auto mt-2 md:mt-0"><p class="text-sm font-bold">Status: <span class="${r.status === 'Pending' ? 'text-orange-500' : r.status === 'Approved' ? 'text-green-600' : 'text-red-600'}">${r.status}</span></p>${r.status === 'Pending' ? `<button class="resolve-id-btn bg-green-500 text-white px-3 py-1 text-sm rounded" data-action="approve" data-userid="${r.userId}">Approve</button><button class="resolve-id-btn bg-red-500 text-white px-3 py-1 text-sm rounded" data-action="deny" data-userid="${r.userId}">Deny</button>` : ''}</div></div>`).join('') : '<p>No pending ID card requests.</p>'}</div></div>`;
    ...
    async function showAdminSignupRequests() {
        ...
        try {
            ...
            if (currentUser.role === 'HOD') {
             ...
             mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Sign-up Requests</h2><div class="overflow-x-auto"><table class="min-w-full"><thead class="bg-gray-100"><tr><th class="p-2 text-left">Name</th><th class="p-2 text-left">User ID</th><th class="p-2 text-left">Email</th><th class="p-2 text-left">Role</th><th class="p-2 text-left">Department</th><th></th></tr></thead><tbody>${requests.map(r => `<tr class="border-b"><td class="p-2">${r.name}</td><td class="p-2">${r.userId}</td><td class="p-2">${r.email}</td><td class="p-2">${r.role}</td><td class="p-2">${r.department || 'N/A'}</td><td class="p-2 flex gap-2"><button class="resolve-signup-btn bg-green-500 text-white px-2 py-1 text-sm rounded" data-action="approve" data-userid="${r.userId}">Approve</button><button class="resolve-signup-btn bg-red-500 text-white px-2 py-1 text-sm rounded" data-action="deny" data-userid="${r.userId}">Deny</button></td></tr>`).join('')}</tbody></table>${requests.length === 0 ? '<p class="mt-4">No pending sign-up requests.</p>' : ''}</div></div>`;
    ...
    async function showAdminPasswordRequests() {
        ...
        try {
            const requests = await fetch(`${API_BASE_URL}/password-requests`).then(res => res.json());
             mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Password Reset Requests</h2><div class="overflow-x-auto"><table class="min-w-full"><thead class="bg-gray-100"><tr><th class="p-2 text-left">Name</th><th class="p-2 text-left">User ID</th><th></th></tr></thead><tbody>${requests.map(r => `<tr class="border-b"><td class="p-2">${r.userName}</td><td class="p-2">${r.userId}</td><td class="p-2 flex gap-2"><button class="resolve-pw-btn bg-green-500 text-white px-2 py-1 text-sm rounded" data-action="approve" data-userid="${r.userId}">Approve</button><button class="resolve-pw-btn bg-red-500 text-white px-2 py-1 text-sm rounded" data-action="deny" data-userid="${r.userId}">Deny</button></td></tr>`).join('')}</tbody></table>${requests.length === 0 ? '<p class="mt-4">No pending password requests.</p>' : ''}</div></div>`;
    ...
    async function showStudentAnnouncements() {
        ...
        try {
            ...
            const announcementsHtml = announcements.length > 0 ? announcements.map(a => `<div class="border p-4 rounded-md bg-gray-50"><p>${a.text}</p><p class="text-xs text-gray-500 mt-2">Posted by ${a.authorName} (${a.authorRole}) on ${new Date(a.date).toLocaleString()}</p></div>`).join('') : '<p>No announcements right now.</p>';

            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Announcements</h2><div class="space-y-4">${announcementsHtml}</div></div>`;
        } catch (error) {
    ...
    async function showFacultyAnnouncements() {
        ...
        try {
            ...
            const announcementsHtml = announcements.length > 0 ? announcements.map(a => `<div class="border p-4 rounded-md bg-gray-50"><p>${a.text}</p><p class="text-xs text-gray-500 mt-2">Posted by ${a.authorName} (${a.authorRole}) on ${new Date(a.date).toLocaleString()}</p></div>`).join('') : '<p>No announcements in your feed.</p>';

            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Announcements</h2><div class="mb-6 p-4 border rounded-lg bg-gray-50"><h3 class="text-lg font-semibold mb-2">Create a New Announcement for Students</h3><form id="announcement-form"><textarea id="announcement-text" class="w-full p-2 border rounded" placeholder="Type new announcement here..."></textarea><button type="submit" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg">Post to Students</button></form></div><h3 class="text-xl font-bold mb-4">Your Feed</h3><div class="space-y-4">${announcementsHtml}</div></div>`;

            document.getElementById('announcement-form').addEventListener('submit', async e => {
                e.preventDefault();
                const text = document.getElementById('announcement-text').value;
                if (!text) return;
                const payload = { text, authorName: user.name, authorRole: user.role, scope: 'students' };
                try {
    ...
    async function showHODAnnouncements() {
        ...
        try {
            ...
            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Department Announcements</h2><div class="mb-6 p-4 border rounded-lg bg-gray-50"><h3 class="text-lg font-semibold mb-2">Create a New Announcement</h3><form id="announcement-form"><textarea id="announcement-text" class="w-full p-2 border rounded mb-2" placeholder="Type new announcement here..."></textarea><label for="announcement-scope" class="block text-sm font-medium text-gray-700">Audience:</label><select id="announcement-scope" class="w-full md:w-1/2 p-2 border rounded-md shadow-sm mb-2"><option value="department_students">Department Students Only</option><option value="department_all">Department Faculty & Students</option></select><button type="submit" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg">Post Announcement</button></form></div><h3 class="text-xl font-bold mb-4">Your Feed</h3><div class="space-y-4">${announcementsHtml}</div></div>`;
            ...
            document.getElementById('announcement-form').addEventListener('submit', async e => {
                ...
                if (!text) return;
                const payload = { text, authorName: user.name, authorRole: user.role, scope: scope, department: user.department };
                try {
                    const res = await fetch(`${API_BASE_URL}/announcements`, {
                        method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload)
                    });
    ...
    async function showAdminAnnouncements() {
        ...
        try {
            ...
            const announcements = await response.json();
            mainContent.innerHTML = `<div class="bg-white p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">Manage All Announcements</h2><form id="announcement-form" class="mb-6 p-4 border rounded-lg bg-gray-50"><textarea id="announcement-text" class="w-full p-2 border rounded" placeholder="Type new announcement here..."></textarea><button type="submit" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg">Post to Everyone</button></form><div class="space-y-4">${announcements.map((a) => `<div class="border p-4 rounded-md flex justify-between items-start"><div><p>${a.text}</p><p class="text-xs text-gray-500 mt-1">By: ${a.authorName} (${a.authorRole}) | Scope: ${a.scope} ${a.department ? `(${a.department})` : ''} | On: ${new Date(a.date).toLocaleString()}</p></div><button class="delete-announcement-btn text-red-500 hover:text-red-700" data-id="${a.id}">Delete</button></div>`).join('')}</div></div>`;
        
            document.getElementById('announcement-form').addEventListener('submit', async e => {
                e.preventDefault();
                const text = document.getElementById('announcement-text').value;
                if (!text) return;
                const payload = { text, authorName: user.name, authorRole: user.role, scope: 'all' };
                try {
                    const res = await fetch(`${API_BASE_URL}/announcements`, {
                        method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload)
                    });
